...
var _this={
		upload:function(file){
			...
			var t=new FormData;
				t.append("file",file);
				t.append("fname",this.randomString(2)+"."+getFileExt(file));
			...
			xhr.onload = () =>
				{
					...
					uploading=false;
					rst(file,domain+"/uploads/"+t.get("fname"));
				}
			xhr.error = () =>
				{
					uploading=false;
...
const ext = new Set(["ICON","BMP","GIF","JPEG","PNG","ICO","JPG","DIB","JFIF","PJPEG","PJP"]);
function setImgs(value) 
{ 
...
function addImg(url){
	document.getElementById("main").innerHTML+="<div class='imgcards'><a title='点击复制链接' href='javascript:void(0)' onclick='clip(\""+url+"\")'><image src='"+url+"' class='hoverwhite resimg'></a><a class='close' style='display:none'href='javascript:rmvImg(\""+url+"\")'><i class='material-icons' style='position:absolute;z-index:10;top:0px;right:0px;background-color:red;color:white'>close</i></a></div>"
	$(".imgcards").hover(
		function(){
